...
defmodule AriaPlanner.Planner.Temporal.STN.Scheduling do
  @moduledoc """
  ...
  @doc """
  Merges overlapping intervals into the minimal set of non-overlapping intervals.

  Used for simplifying timeline analysis by eliminating redundancy.
  """
  @spec merge_overlapping_intervals([map()]) :: [map()]
  def merge_overlapping_intervals([]), do: []

  def merge_overlapping_intervals(intervals) do
  ...
  @doc """
  Calculates gaps between intervals up to the specified total time limit.

  Returns a list of free time slots (gaps) between the intervals.
  Useful for finding available time slots in a constrained schedule.
  """
  @spec calculate_interval_gaps([map()], number()) :: [map()]
  def calculate_interval_gaps(intervals, total_time) do
    sorted_intervals = Enum.sort_by(intervals, & &1.start_time)
    gaps = []

    # Gap from start of time range to first interval
    gaps =
      case sorted_intervals do
        [] ->
          gaps

        [first | _] ->
          if first.start_time > 0 do
            [%{start_time: 0, end_time: first.start_time} | gaps]
          else
            gaps
          end
      end

    # Gaps between adjacent intervals
    gaps =
      Enum.reduce(Enum.zip(sorted_intervals, Enum.drop(sorted_intervals, 1)), gaps, fn {current, next}, acc ->
        gap_start = current.end_time
        gap_end = next.start_time

        if gap_end > gap_start do
          [%{start_time: gap_start, end_time: gap_end} | acc]
        else
          acc
        end
      end)

    # Gap from last interval to total_time
    gaps =
      case List.last(sorted_intervals) do
        nil ->
          gaps

        last ->
          if last.end_time < total_time do
            [%{start_time: last.end_time, end_time: total_time} | gaps]
          else
            gaps
          end
      end
  ...
  defp find_gaps_in_timeline(occupied_intervals, window_start, window_end, required_duration) do
    ...
    gaps =
      Enum.reduce(Enum.zip(merged_intervals, Enum.drop(merged_intervals, 1)), gaps, fn {current, next}, acc ->
        gap_start = current.end_time
        gap_end = next.start_time
