...
defmodule AriaPlanner.Planner.Temporal.STN.Units do
  ...
  @type lod_resolution :: 1 | 10 | 100 | 1_000 | 10_000
  ...
  def rescale_lod(stn, new_lod_level) do
    if stn.lod_level == new_lod_level do
      ...
      if MapSet.size(updated_stn.time_points) > 0 do
        case AriaStnSolver.solve_stn(updated_stn) do
  ...
  @doc """
  Converts ISO 8601 duration string to STN units.

  For variable durations (ranges), returns {min, max} tuple.
  For fixed durations, returns {duration, duration}.
  """
  @spec convert_iso8601_duration_to_stn_units(
          String.t(),
          time_unit(),
          lod_level(),
          lod_resolution()
        ) :: {number(), number()} | {:error, String.t()}
  def convert_iso8601_duration_to_stn_units(
        iso_duration,
        target_unit,
        _lod_level,
        lod_resolution
      ) do
    case parse_iso8601_duration_to_microseconds(iso_duration) do
      {:ok, {:fixed, microseconds}} ->
        target_unit_microseconds = unit_to_microseconds(target_unit)
        duration_in_target_units = microseconds / target_unit_microseconds
        duration_in_stn_units = duration_in_target_units * lod_resolution
        rounded = round(duration_in_stn_units)
        {rounded, rounded}

      {:ok, {min_microseconds, max_microseconds}} ->
        target_unit_microseconds = unit_to_microseconds(target_unit)
        min_duration_in_target_units = min_microseconds / target_unit_microseconds
        max_duration_in_target_units = max_microseconds / target_unit_microseconds
        min_duration_in_stn_units = min_duration_in_target_units * lod_resolution
        max_duration_in_stn_units = max_duration_in_target_units * lod_resolution
        {round(min_duration_in_stn_units), round(max_duration_in_stn_units)}
  ...
  @doc """
  Converts microseconds to STN units.
  """
  @spec convert_microseconds_to_stn_units(integer(), time_unit(), lod_resolution()) :: number()
  def convert_microseconds_to_stn_units(microseconds, target_unit, lod_resolution) do
    target_unit_microseconds = unit_to_microseconds(target_unit)
    units = microseconds / target_unit_microseconds
    round(units * lod_resolution)
  end

  # Helper function to parse ISO 8601 duration to microseconds
  defp parse_iso8601_duration_to_microseconds(iso_duration) when is_binary(iso_duration) do
    if String.contains?(iso_duration, "/") do
      # Handle variable duration (range)
      [min_iso_duration, max_iso_duration] = String.split(iso_duration, "/")
