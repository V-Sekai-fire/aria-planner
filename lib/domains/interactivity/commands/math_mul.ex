...
defmodule AriaPlanner.Domains.Interactivity.Commands.MathMul do
  @moduledoc """
  Command: c_math_mul(node_id, a_socket, b_socket, value_socket)

  Executes math/mul operation: value = a * b
  Operates component-wise for floatN and floatNxN types (per-element multiplication).
  ...
  def c_math_mul(state, node_id, a_socket, b_socket, value_socket) do
    with :ok <- check_graph_active(state),
      ...
      # Compute result (per-element multiplication)
  ...
  defp multiply_values(a, b) when is_number(a) and is_number(b), do: a * b
  defp multiply_values({a1, a2}, {b1, b2}), do: {a1 * b1, a2 * b2}
  defp multiply_values({a1, a2, a3}, {b1, b2, b3}), do: {a1 * b1, a2 * b2, a3 * b3}
  defp multiply_values({a1, a2, a3, a4}, {b1, b2, b3, b4}), do: {a1 * b1, a2 * b2, a3 * b3, a4 * b4}

  defp multiply_values(a, b) when is_tuple(a) and is_tuple(b) do
    # Handle matrix types (per-element multiplication)
    Tuple.to_list(a)
  ...
  defp multiply_values(a, b) do
    # Fallback: try to multiply as numbers
    a * b
  rescue
