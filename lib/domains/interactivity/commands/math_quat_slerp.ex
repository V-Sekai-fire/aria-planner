...
defmodule AriaPlanner.Domains.Interactivity.Commands.MathQuatSlerp do
  @moduledoc """
  Command: c_math_quat_slerp(node_id, ...)

  Executes math/quatSlerp operation.
  ...
  def c_math_quat_slerp(state, node_id, a_socket, b_socket, c_socket, value_socket) do
    case MathHelpers.check_graph_active(state) do
      :ok ->
        with {:ok, a} <- MathHelpers.get_socket_value(state, node_id, a_socket),
             ...
             {:ok, t} <- MathHelpers.get_socket_value(state, node_id, c_socket) do
          result = MathHelpers.quat_slerp_op(a, b, t)
