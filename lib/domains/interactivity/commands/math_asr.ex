...
defmodule AriaPlanner.Domains.Interactivity.Commands.MathAsr do
  @moduledoc """
  Command: c_math_asr(node_id, ...)

  Executes math/asr operation.
  ...
  def c_math_asr(state, node_id, a_socket, _b_socket, value_socket) do
    with :ok <- MathHelpers.check_graph_active(state),
         {:ok, a} <- MathHelpers.get_socket_value(state, node_id, a_socket),
         {:ok, b} <- MathHelpers.get_socket_value(state, node_id, a_socket) do
      # Compute asr (component-wise)
      result = MathHelpers.apply_binary_op(a, b, &MathHelpers.asr_op/2)
