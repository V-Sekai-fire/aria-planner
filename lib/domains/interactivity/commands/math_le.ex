...
defmodule AriaPlanner.Domains.Interactivity.Commands.MathLe do
  @moduledoc """
  Command: c_math_le(node_id, a_socket, b_socket, value_socket)

  Executes math/le operation: value = a <= b
  Compares a <= b component-wise using MathHelpers.le_op.
  ...
  def c_math_le(state, node_id, a_socket, b_socket, value_socket) do
    with :ok <- MathHelpers.check_graph_active(state),
      ...
      # Compare a <= b using MathHelpers.le_op for component-wise operation
      result = MathHelpers.apply_binary_op(a, b, &MathHelpers.le_op/2)
