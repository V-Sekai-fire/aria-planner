...
defmodule AriaPlanner.Domains.Interactivity.Commands.MathClamp do
  @moduledoc """
  Command: c_math_clamp(node_id, ...)

  Executes math/clamp operation.
  ...
  def c_math_clamp(state, node_id, a_socket, b_socket, c_socket, value_socket) do
    with :ok <- MathHelpers.check_graph_active(state),
      ...
      # Compute clamp: min(max(a, min(b, c)), max(b, c))
      result = MathHelpers.clamp_op(a, b, c)
