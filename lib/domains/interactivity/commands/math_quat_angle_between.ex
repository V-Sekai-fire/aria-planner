...
defmodule AriaPlanner.Domains.Interactivity.Commands.MathQuatAngleBetween do
  @moduledoc """
  Command: c_math_quat_angle_between(node_id, ...)

  Executes math/quatAngleBetween operation.
  ...
  @spec c_math_quat_angle_between(
          state :: map(),
          node_id :: String.t(),
          a_socket :: String.t(),
          b_socket :: String.t(),
  ...
  def c_math_quat_angle_between(state, node_id, a_socket, b_socket, value_socket) do
    case MathHelpers.check_graph_active(state) do
      :ok ->
        with {:ok, a} <- MathHelpers.get_socket_value(state, node_id, a_socket),
          ...
          result = MathHelpers.quat_angle_between_op(a, b)
