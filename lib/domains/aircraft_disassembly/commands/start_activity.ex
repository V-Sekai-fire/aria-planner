...
defmodule AriaPlanner.Domains.AircraftDisassembly.Commands.StartActivity do
  ...
  defp check_resource_skill_requirements(state, activity, assigned_resources) do
    num_skills = Map.get(state, :num_skills, 1)
    skill_reqs = Map.get(state, :skill_requirements, [])
  ...
  defp check_location_capacity(state, activity, start_datetime) do
    ...
    overlapping_activities = find_overlapping_activities_at_location(state, location, start_datetime, 0)
  ...
  defp get_skill_requirement(skill_reqs, activity, skill_idx, state) do
    num_skills = Map.get(state, :num_skills, 1)
    idx = (activity - 1) * num_skills + (skill_idx - 1)
    Enum.at(skill_reqs, idx, 0)
  end
