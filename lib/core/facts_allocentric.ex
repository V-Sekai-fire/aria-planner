...
defmodule AriaCore.FactsAllocentric do
  ...
  # Ego projections (persona beliefs stored as facts - may differ)
  {:ok, belief_facts} = FactsAllocentric.get_facts_about(persona_a.id)
  ego_beliefs = Enum.filter(belief_facts, &(&1.predicate =~ "believes"))
  ...
  def query_observable(observer_persona_id, target_entity_id)
    ...
    case get_facts_about(target_entity_id) do
      {:ok, facts} ->
        observable_facts = facts |> Enum.filter(fn fact -> fact.fact_type in observable_types end)
        {:ok, observable_facts}
  ...
  def validate_world_state do
    ...
    active_facts =
      all()
      |> Enum.filter(fn fact ->
        fact.expires_at == nil or DateTime.compare(fact.expires_at, DateTime.utc_now()) == :gt
      end)
  ...
  def conflicting_facts(subject_id) when is_binary(subject_id) do
    case get_facts_about(subject_id) do
      {:ok, facts} ->
