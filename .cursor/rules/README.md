...
This directory contains workspace-level rules that guide development practices,
coding standards, and communication style for the aria-planner project.
...
- `commit-message-style.mdc` - General style requirements (no conventional
  commits)
- `commit-message-completeness-check.mdc` - Mandatory completeness verification
- `telegraph-style-commit-messages.mdc` - Implementation detail for concise
  messaging
...
- `simple-solutions-for-simple-problems.mdc` - Match solution complexity to
  problem
- `targeted-solutions-over-generalized-systems.mdc` - Single responsibility
  principle
...
- `alwaysApply: true` - Rules that are always applied
...
When referencing rules, use the filename (e.g.,
`telegraph-style-commit-messages.mdc`) rather than ADR IDs or other identifiers.

## Adding New Rules

1. **Choose the appropriate category** based on the rule's focus
1. **Use descriptive filenames** with kebab-case (e.g., `my-new-rule.mdc`)
1. **Include metadata** at the top with `alwaysApply: true` and a description
1. **Cross-reference** related rules using filenames
...
- **Natural communication**: Avoid conventional commit prefixes, use telegraph
  style
